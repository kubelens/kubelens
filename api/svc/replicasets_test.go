...
func TestGetReplicaSets(t *testing.T) {
	h := getSvc()
	req := httptest.NewRequest("GET", `/replicasets?namespace=test&linkedName=appname"`, nil)
	...
	var b []k8sv1.ReplicaSetOverview
...
func TestGetReplicaSet(t *testing.T) {
	h := getSvc()
	req := httptest.NewRequest("GET", "/replicasets/test?namespace=test", nil)
	w := httptest.NewRecorder()
